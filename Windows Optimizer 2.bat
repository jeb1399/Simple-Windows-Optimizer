...
set services[14]=TabletInputService
set services[15]=lfsvc
set services[16]=ShellHWDetection
set services[17]=AarSvc
set services[18]=CscService
set services[19]=DisplayEnhancementService
set services[20]=MicrosoftEdgeElevationService
set services[21]=edgeupdate
set services[22]=edgeupdatem
set serviceCount=23
...
echo     Windows telemetry - Data collection (thats all I know) !! Cant change this option sorry !!
echo     Windows Customer Experience Program - Another data collection service that collects information about how you use windows !! Cant change this option sorry !!
echo     SMBv1 - This was literally used by wannacry idk why you would still want this !! Cant change this option sorry !!
echo     CTF Loader - Handwriting detection service. If you dont have a handwriting device or anything similar you dont need this enabled.
echo     Geolocation Services - Does anyone really use GPS on a COMPUTER?
echo     Shell Hardware Detection - Appearently its now uncommon for people to use DVDs/CDs anymore (Disables AutoPlay)
echo     Agent Activation - Oh my god its cortonas corpse burried in windows. Will disable voice assistants.
echo     Offline Files - No this does not mean you will no longer be able to access your files however it will spike because of OneDrive forcefully syncing its files on your computer.
echo     Display Enhancement Service - Disables windows HDR (its really bad).
echo     MS Edge Elevation Service - Why does edge need administrative priviledges if nobody wants it?
echo     MS Edge Update service (edgeupdate and edgeupdatem) - Keeps edge updated automatically.
echo.
...
echo     - Help
echo.
echo The following legacy (old) programs will be removed:
echo     - winhlp32.exe
echo     - hh.exe
echo     - write.exe
echo.
echo Additionally removed programs/temp files:
echo     - Update leftovers
echo     - Windows tips
echo     - Temp files
echo.
...
if errorlevel 1 (
    ...
    del /f /s /q %TEMP%\* >nul 2>&1 & del /f /s /q C:\Windows\Temp\* >nul 2>&1

    rmdir /s /q "C:\Program Files (x86)\Microsoft\Edge" > NUL 2>&1
    rmdir /s /q "C:\Program Files (x86)\EdgeUpdate" > NUL 2>&1

    takeown /F "C:\Windows\HelpPane.exe" >nul 2>&1
    takeown /F "C:\Windows\winhlp32.exe" >nul 2>&1
    takeown /F "C:\Windows\hh.exe" >nul 2>&1
    takeown /F "C:\Windows\write.exe" >nul 2>&1
    takeown /F "C:\Windows\Help" >nul 2>&1

    icacls "C:\Windows\HelpPane.exe" /grant Everyone:F >nul 2>&1
    icacls "C:\Windows\winhlp32.exe" /grant Everyone:F >nul 2>&1
    icacls "C:\Windows\hh.exe" /grant Everyone:F >nul 2>&1
    icacls "C:\Windows\write.exe" /grant Everyone:F >nul 2>&1
    icacls "C:\Windows\Help" /grant Everyone:F >nul 2>&1

    move "C:\Windows\HelpPane.exe" "C:\Windows\HelpPane.exe.unused"
    move "C:\Windows\winhlp32.exe" "C:\Windows\winhlp32.exe.unused"
    move "C:\Windows\hh.exe" "C:\Windows\hh.exe.unused"
    del /q "C:\Windows\write.exe"
